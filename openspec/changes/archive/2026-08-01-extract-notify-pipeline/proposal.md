...
## Known collision — RESOLVED (2026-07-31, Leo)

herdr-shepherd's open change `add-notify-voice-management` is split, not parked: its
surface-agnostic data layer (sections 1, 2, 4.2) is retargeted here as herald change
`voice-management-core`; the dock UI half (section 3) stays in herdr-shepherd as a
slimmed change consuming herald's CLI seams, still blocked on the herdr fork's
`placement = "dock"`. When executing this extraction, do NOT delete `pkg/notify` from
herdr-shepherd until the slimming edit of that change's proposal (voice-management-core
task 3.1) is ready to land in the same window.
