...
- [x] 1.1 Create `plugin/.claude-plugin/plugin.json` and `marketplace.json` plus
...
- [x] 1.2 Move cc `scripts/lib/notify.sh` → `plugin/lib/notify.sh` unchanged except the
...
- [x] 1.3 Move cc `output-styles/tts-summary.md` → `plugin/output-styles/` and inject it
...
- [x] 2.1 `bin/notify.sh`: before synthesis, if `$HERALD_STATE_DIR/mute` exists and its
...
- [x] 3.1 Author `plugin/commands/notify.md` with subcommands status / history / mute /
...
- [x] 3.2 `/notify test` runs the pipe with `--wait` and prints the resulting history
      line. Verify: audible + record shown.

## 4. cc spec + caller cleanup

- [x] 4.1 Rewrite cc `openspec/specs/notify-command/spec.md` against the herald backend:
      ...
      grep scenarios pass.
- [x] 4.2 Collapse `orch_tts_notify` (cc
...
- [x] 5.1 Session transcript snippet: `/notify mute 1h` → silent `say_notify` → `muted`
